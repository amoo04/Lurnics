...
export default function Footer() {
  ...
  return (
    <footer className="border-t border-white/10 px-4 py-12 text-sm text-gray-400 sm:px-8 lg:px-20">
      <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 md:grid-cols-[1.3fr_1fr_1fr_1fr_1.2fr]">
        <div>
          <div className="h-9 w-[160px] overflow-hidden">
            <img
              src={logo}
              alt="Lurnics"
              className="h-9 w-[160px] object-cover"
            />
          </div>
        ...
        <div>
          ...
          {status === "success" ? (
            <p className="mt-4 flex items-center gap-2 text-xs text-indigo-300">
              <CheckCircle2 size={14} />
              You're subscribed!
            </p>
          ) : (
            <form onSubmit={handleSubmit} className="mt-4 flex items-center gap-2 rounded-md border border-white/10 px-3 py-2">
              <input
                type="email"
                required
                value={email}
